...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            res.status(201).json({message: "user registered", data, code: code.code});
...
router.post("/update-password", authenticate, validate(userValidationSchema.updatePassword), async (req, res) => {
    try {
        const data = await UserService.updatePassword({_id: req.user._id, ...req.body});
...
router.patch("/update-profile", authenticate, validate(userValidationSchema.updateProfile), async (req, res) => {
    try {
